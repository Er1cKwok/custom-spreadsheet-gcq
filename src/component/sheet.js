 ...
 * Author: GCQ
 * 3月31号修改
 * 1. 修改insertDeleteRowColumn方法（在excel表录入数据后，禁止删除行，插入原来列，删除原来列等）
 */
...
function insertDeleteRowColumn(type) {
  const { data } = this;
  const currentCol = sessionStorage.getItem('currentCol');
  const lastColIndex = sessionStorage.getItem('lastColIndex');
  const ifImport = +sessionStorage.getItem('ifImport');
  ...
  if (type === 'insert-row') {
    ...
    if (ifImport && currentCol <= lastColIndex) {
      Message.error('已录入数据的模板不能改变原有的表头格式');
      return;
    }
    data.insert('column');
